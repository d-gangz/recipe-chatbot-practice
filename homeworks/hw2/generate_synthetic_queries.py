...
def generate_queries_parallel(
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        future_to_tuple = {
            # Each DimensionTuple in the list has no built-in ID, so
            # we pair the Future with its position (i) in dimension_tuples.
            # Later, when the Future finishes, we grab the tuple again via that index.
